...
void merge_sort(array arr, int size)
{
    // 每次合并的间隔,逐次乘以2
    int gap = 1;
    // 定义一些中间变量,左数组起始、右数组起始、左数组结尾(不包括）,右数组结尾(不包括）,辅助数组该记哪个数
    int left, right, tmp_left, tmp_right, count;
    // 申请额外空间,辅助数组
    ptr_int32 tmp_arr = (ptr_int32)(malloc(sizeof(int) * size));
    // 初始化数组
    memset(tmp_arr, 0, sizeof(int) * size);
    ...
    while (gap < size)
    {
        ...
        for (int i = 0; i < size - gap; i += 2 * gap)
        {
            ...
            fori(0, 2 * gap)
            {
                ...
                {
                    for (int j = left; j < tmp_left && j < size; j++)
                    {
                        tmp_arr[count] = arr[j];
                        count++;
                    }
                ...
                {
                    for (int j = right; j < tmp_right && j < size; j++)
                    {
                        tmp_arr[count] = arr[j];
                        count++;
                    }
